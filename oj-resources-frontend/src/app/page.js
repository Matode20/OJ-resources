    ...
    <div className="min-h-screen bg-gray-100">
      ...
      >
        ...
        <div className="relative flex flex-col items-center justify-center z-10 px-4">
          ...
          <p className="mt-4 text-lg md:text-xl max-w-2xl justify-center text-white">
            High-quality organic produce delivered fast & fresh. Shop now and
            enjoy healthy living!
          </p>
          <Link href="/Shop">
            <Button className="mt-6 bg-gray-500 text-black hover:bg-green-500 hover:text-white px-6 py-3 text-lg font-semibold rounded-xl">
              Shop Now
            </Button>
          </Link>
        </div>
      ...
      {/* Featured Products */}
      <section className="py-12 px-6 text-center animate-pulse">
        <h2 className="text-3xl font-semibold">Featured Products</h2>
        <div className="mt-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((item) => (
            <div key={item} className="bg-white p-4 shadow-md animate-pulse rounded-lg">
